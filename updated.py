...
NOTE_FILES = {note: f"audio/{note}.wav" for note in NOTE_NAMES}
INTERVALS = [
...
class EarTraining:
    ...
    def start(self):
        ...
        # ✅ NEW THEORY SECTION
        theory_frame = tk.LabelFrame(self.root, text="📘 Theory", padx=10, pady=5)
        theory_frame.pack(pady=5)
        self.theory_feedback = tk.Label(theory_frame, text="", justify="left", wraplength=700)
        self.theory_feedback.pack()
        theory_btns = tk.Frame(theory_frame)
        theory_btns.pack()
        tk.Button(theory_btns, text="🎼 Notes", command=self.show_note_theory).pack(side=tk.LEFT, padx=5)
        tk.Button(theory_btns, text="🎹 Chords", command=self.show_chord_theory).pack(side=tk.LEFT, padx=5)
        tk.Button(theory_btns, text="📏 Scales", command=self.show_scale_theory).pack(side=tk.LEFT, padx=5)
    ...
    # ✅ THEORY SECTION FUNCTIONS
    def show_note_theory(self):
        self.theory_feedback.config(text=(
            "🎼 **Notes Theory**\n"
            "Notes are the basic building blocks of music. Each note represents a specific pitch.\n"
            "- Examples: C4, D#4, F5\n"
            "- Sharps (#) raise the pitch by a semitone.\n"
            "- Flats (b) lower the pitch by a semitone.\n"
            "Octaves are numbered: C4 is middle C, C5 is one octave above."
        ))

    def show_chord_theory(self):
        self.theory_feedback.config(text=(
            "🎹 **Chord Theory**\n"
            "Chords are made by stacking multiple notes played together.\n"
    ...
    def show_scale_theory(self):
        self.theory_feedback.config(text=(
            "📏 **Scale Theory**\n"
            "Scales are sequences of notes in ascending or descending order.\n"
            "- Major: W-W-H-W-W-W-H\n"
            "- Natural Minor: W-H-W-W-H-W-W\n"
            "- Pentatonic: Five notes per octave\n"
